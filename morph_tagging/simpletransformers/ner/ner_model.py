...
class NERModel:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        Args:
            ...
            only_muna (optional): Train model only on word "muna". If `'muna'` given, will completely ignoring non-"muna" tokens. If `'all'` given, will include others in the loss with a smaller weight Defaults to False.
        ...
        **kwargs,
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if self.args.fp16:
                    with amp.autocast():
                        if only_muna and only_muna == 'muna':
                            loss, *_ = self._calculate_loss_muna(
                                model,
                                inputs,
                        ...
                        elif only_muna and only_muna == 'all':
                            loss, *_ = self._calculate_loss_muna_all(
                                model,
                                inputs,
                                loss_fct=self.loss_fct,
                                num_labels=self.num_labels,
    ...
    def _calculate_loss_muna(self, model, inputs, loss_fct, num_labels, args):
        """
        Custom loss function that only penalizes incorrect labeling of the word 'muna'.
        """
        outputs = model(**inputs)
        logits = outputs[1]
        labels = inputs["labels"]
        ...
        if loss_fct:
            ...
            # Identify 'muna' tokens
            token_ids = inputs.get("input_ids")
            word_ids = self.tokenizer.convert_ids_to_tokens(token_ids.view(-1))  # Flatten token IDs
            is_muna = torch.tensor(
                [1 if "▁muna" in word else 0 for word in word_ids], device=labels.device
            ).type_as(labels)

            # Mask for 'muna' tokens
            muna_mask = (is_muna == 1) & active_loss
            active_logits = active_logits[muna_mask]
            active_labels = active_labels[muna_mask]

            # Calculate loss only for 'muna'
            if muna_mask.sum() > 0:
                loss = loss_fct(active_logits, active_labels)
            else:
                # Assign zero loss for batches without 'muna', ensuring proper gradient flow
                loss = torch.tensor(0.0, device=logits.device, requires_grad=True)
        else:
            loss = outputs[0]  # Use the model's internal loss if no custom loss function

        return (loss, *outputs[1:])

    def _calculate_loss_muna_all(self, model, inputs, loss_fct, num_labels, args):
        """
        Custom loss function: Penalizes incorrect 'muna' predictions while weighting non-'muna' context.
        """
        outputs = model(**inputs)
        logits = outputs[1]
        labels = inputs["labels"]
        ...
        if loss_fct:
            ...
            # Identify 'muna' tokens
            token_ids = inputs.get("input_ids")
            word_ids = self.tokenizer.convert_ids_to_tokens(token_ids.view(-1))
            is_muna = torch.tensor(
                [1 if "▁muna" in word else 0 for word in word_ids], device=labels.device
            ).type_as(labels)

            # Weighted loss
            weights = torch.ones_like(active_labels, dtype=torch.float32)
            weights[is_muna == 1] *= 10.0  # Higher weight for 'muna'
            loss = torch.nn.functional.cross_entropy(active_logits, active_labels, weight=weights)

        else:
            loss = outputs[0]  # Use model's internal loss if no loss function provided
