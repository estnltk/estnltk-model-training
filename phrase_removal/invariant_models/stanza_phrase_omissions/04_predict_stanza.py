...
#       * smaller_data
#
#    Note: due to technical reasons, you should download 
#    the original stanza's "et" models before running 
#    this script first time:
#
#    $ python -c "import stanza; stanza.download(lang='et')"
#
#    Source based on:
#    https://github.com/estnltk/estnltk-model-training/tree/main/stanza_syntax_training
#
import os, os.path
import re
import sys
...
def predict_with_stanza(input_path, model_path, output_path, lang='et', use_gpu=False):
    ...
    Note: even though this function does not need to download or use stanza's "et" 
    models, you may run into obscure indexing errors if you attempt to run this in 
    an environment where stanza's "et" models (corresponding to the current stanza 
    version) are missing. Therefore, you should download the default models before-
    hand:
            >> import stanza; stanza.download(lang='et')
