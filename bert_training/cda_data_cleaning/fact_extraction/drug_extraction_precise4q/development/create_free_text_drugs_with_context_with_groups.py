...
cur.execute("""SELECT * from work.free_text_drugs_with_context;""")
...
for idx, row in enumerate(tagged_rows):
    ...
    if row[2] == "":
        ...
        for drug in p4q_dict:
            ...
            if dist == 0:
                dist0_rows.append([row[0], cleaned_row_drug, drug, p4q_dict[drug], row[3], row[4], row[5]])
            elif dist == 1:
                dist1_rows.append([row[0], cleaned_row_drug, drug, p4q_dict[drug], row[3], row[4], row[5]])
            elif len(row[1]) > 6 and len(drug) > 6 and dist == 2:
                dist2_rows.append([row[0], cleaned_row_drug, drug, p4q_dict[drug], row[3], row[4], row[5]])
            elif len(row[1]) > 8 and len(drug) > 8 and dist == 3:
                dist3_rows.append([row[0], cleaned_row_drug, drug, p4q_dict[drug], row[3], row[4], row[5]])
            elif len(row[1]) > 10 and len(drug) > 10 and dist == 4:
                dist4_rows.append([row[0], cleaned_row_drug, drug, p4q_dict[drug], row[3], row[4], row[5]])
    else:
        new_row = [row[0], row[1], row[2], "statin", row[3], row[4], row[5]]
...
cur.execute(
    """
drop table if exists work.free_text_drugs_with_context_with_p4q_groups;
create table work.free_text_drugs_with_context_with_p4q_groups (epi_id text, drug_text text, drug_standard text, drug_p4q text, context text, "table" text, field text);"""
)

for row in ok_rows_unique:
    cur.execute(
        "insert into work.free_text_drugs_with_context_with_p4q_groups VALUES (%s, %s, %s, %s, %s, %s, %s)",
        (row[0], row[1], row[2], row[3], row[4].strip(), row[5], row[6]),
    )
