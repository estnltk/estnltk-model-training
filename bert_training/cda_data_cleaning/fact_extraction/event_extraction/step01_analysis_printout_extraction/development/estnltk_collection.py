...
class PgCollection:
    ...
    def create(self, description=None, meta: dict = None, temporary=None):
        ...
        pg.create_collection_table(
            self.storage, collection_name=self.name, meta_columns=self.meta, description=description
        )
    ...
    def selected_layers(self):
        if self._selected_layes is None:
            if self._is_empty is None:
                return []
            self._selected_layes = [
                layer
                for layer, properties in self._structure.structure.items()
    ...
    def dependent_layers(self, selected_layers):
        ...
        for layer in selected_layers:
            if layer not in self._structure:
                raise pg.PgCollectionException(
                    "there is no layer {!r} in the collection {!r}".format(layer, self.name)
                )
            include_dep(layer)
        ...
        with self.storage.conn.cursor() as c:
            c.execute(
                SQL("DROP INDEX {schema}.{index}").format(
                    schema=Identifier(self.storage.schema), index=Identifier("idx_%s_data" % self.name)
                )
            )
        ...
        with self.storage.conn.cursor() as cursor:
            cursor.execute(
                SQL("INSERT INTO {} SELECT * FROM {}").format(
                    pg.collection_table_identifier(self.storage, self.name),
    ...
    def _insert_first(self, text, key, meta_data, cursor, table_identifier, column_identifiers):
        ...
        self._flush_insert_buffer(
            cursor=cursor, table_identifier=table_identifier, column_identifiers=column_identifiers, buffer=[query]
        )
        ...
        with self.storage.conn.cursor() as cursor:
            ...
            def wrap_buffered_insert(text, key=None, meta_data=None):
                ...
                if any(struct["layer_type"] == "detached" for struct in self._structure.structure.values()):
                    ...
                    assert set(text.layers) == set(self._structure), "{} != {}".format(
                        set(text.layers), set(self._structure)
                    )
            ...
            try:
                ...
                logger.info(
                    "inserted {self._insert_counter} texts into the collection {self.name!r}".format(self=self)
                )
        ...
        try:
            cursor.execute(
                SQL("INSERT INTO {} ({}) VALUES {};").format(
                    table_identifier, column_identifiers, SQL(", ").join(buffer)
                )
            )
            ...
            raise
        logger.debug(
            "flush buffer: {} rows, {} bytes, {} estimated characters".format(
    ...
    def select_fragment_raw(self, fragment_name, parent_layer_name, query=None, ngram_query=None):
        ...
        if ngram_query is not None:
            ...
            ngram_q = SQL(" AND ").join(
                SQL(self._build_column_ngram_query(q, col, fragment_name)) for col, q in ngram_query
            )
            sql_parts.append(SQL("AND"))
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, int):
            ...
            subcollection = self.select(
                query=IndexQuery(item), layers=self.selected_layers, collection_meta=None, return_index=False
            )
        ...
        if isinstance(item, slice):  # TODO
            ...
            return self.select(
                query=SliceQuery(item.start, item.stop),
    ...
    def find_fingerprint(self, query=None, layer_query=None, layer_ngram_query=None, layers=None, order_by_key=False):
        ...
        def build_text_query(q):
            ...
            for and_terms in q["query"]:
                ...
                if and_terms:
                    and_query = reduce(
                        op.__and__,
                        (JsonbTextQuery(q["layer"], q["ambiguous"], **{q["field"]: term}) for term in and_terms),
                    )
        ...
        def build_layer_query(layer_name, q):
            ...
            for and_terms in q["query"]:
                ...
                if and_terms:
                    and_query = reduce(
                        op.__and__,
                        (JsonbLayerQuery(layer_name, q["ambiguous"], **{q["field"]: term}) for term in and_terms),
                    )
                    or_query_list.append(and_query)
            if len(or_query_list) > 0:
                jsonb_query = reduce(op.__or__, or_query_list)
        ...
        jsonb_layer_query = (
            {layer: build_layer_query(layer, q) for layer, q in layer_query.items()}
            if layer_query is not None
            else None
        )

        return self.select(
            query=jsonb_text_query, layer_query=jsonb_layer_query, layer_ngram_query=layer_ngram_query, layers=layers
        )

    def create_fragment(self, fragment_name, data_iterator, row_mapper, create_index=False, ngram_index=None):
                ...
                for row in data_iterator:
                    ...
                    for record in row_mapper(row):
                        ...
                        if ngram_index is not None:
                            ngram_values = [
                                create_ngram_fingerprint_index(record.layer, attr, n)
    ...
    def continue_creating_layer(self, tagger, progressbar=None, query_length_limit=5000000):
        self.create_layer(tagger=tagger, progressbar=progressbar, query_length_limit=query_length_limit, mode="append")

    def create_fragmented_layer(
        self,
        tagger,
        fragmenter: Callable,
        meta: Sequence = None,
        ...
        if not self.exists():
            raise PgCollectionException(
                "collection {!r} does not exist, can't create layer {!r}".format(self.name, layer_name)
            )
        ...
        with conn.cursor() as c:
            try:
                ...
                self._create_layer_table(cursor=c, layer_name=layer_name, meta=meta)

                structure_written = False
                with self.buffered_layer_insert(
                    table_identifier=layer_table_identifier(self.storage, self.name, layer_name),
                    columns=columns,
    ...
    def create_layer(
        ...
        Args:
            ...
            query_length_limit: int
                ...
                insert
        """
        assert (layer_name is None and data_iterator is None and row_mapper is None) is not (
            tagger is None
        ), "either tagger ({}) must be None or layer_name ({}), data_iterator ({}) and row_mapper ({}) must be None".format(
            tagger, layer_name, data_iterator, row_mapper
        )
        ...
        data_iterator = data_iterator or self.select(
            layers=tagger.input_layers,
            progressbar=progressbar,
        ...
        if self.has_layer(layer_name):
            if mode == "overwrite":
                ...
                exception = PgCollectionException(
                    "can't create {!r} layer, the layer already exists".format(layer_name)
                )
                logger.error(exception)
                raise exception
        else:
            ...
            try:
                ...
                with self.buffered_layer_insert(
                    ...
                    for row in data_iterator:
                        ...
                        if ngram_index is not None:
                            values.extend(
                                create_ngram_fingerprint_index(layer=layer, attribute=attr, n=ngram_index[attr])
    ...
    def create_layer_table(self, layer_name, meta=None):
        ...
        if pg.table_exists(self.storage, layer_table):
            raise PgCollectionException(
                "The table {!r} of the {!r} layer already exists.".format(layer_table, layer_name)
            )
        ...
        q = SQL("CREATE TABLE {layer_identifier} ({columns})").format(
            layer_identifier=layer_identifier, columns=columns
        )
        ...
        with self.storage.conn.cursor() as cursor:
            ...
            q = SQL("COMMENT ON TABLE {} IS {};").format(
                layer_identifier, Literal("created by {} on {}".format(self.storage.user, time.asctime()))
            )
            cursor.execute(q)
            logger.debug(cursor.query.decode())
    ...
    def _create_layer_table(
        self, cursor, layer_name, is_fragment=False, create_index=True, ngram_index=None, overwrite=False, meta=None
    ):
        if is_fragment:
        ...
        q = (
            "CREATE {temporary} TABLE {layer_identifier} ("
        ...
        if ngram_index is not None:
            ngram_cols = ", %s" % ",".join(
                ["%s text[]" % Identifier(column).as_string(self.storage.conn) for column in ngram_index]
            )
        else:
            ngram_cols = ""
        ...
        if ngram_index is not None:
            for column in ngram_index:
                cursor.execute(
                    SQL("CREATE INDEX {index} ON {schema}.{table} USING gin ({column});").format(
                        schema=Identifier(self.storage.schema),
        ...
        cursor.execute(
            SQL("CREATE INDEX {index} ON {layer_table} (text_id);").format(
                index=Identifier("idx_%s__text_id" % layer_table), layer_table=layer_identifier
            )
        )
        logger.debug(cursor.query.decode())
        ...
        for ln, struct in self._structure.structure.items():
            ...
            if layer_name == struct["enveloping"] or layer_name == struct["parent"]:
                if cascade:
                    ...
                    raise PgCollectionException(
                        "can't delete layer {!r}; " "there is a dependant layer {!r}".format(layer_name, ln)
                    )
        drop_layer_table(self.storage, self.name, layer_name)
    ...
    def export_layer(self, layer, attributes, collection_meta=None, table_name=None, progressbar=None):
        ...
        with self.storage.conn.cursor() as c:
            ...
            c.execute(SQL("CREATE TABLE {} ({});").format(table_identifier, columns_sql))
            logger.debug(c.query)
            c.execute(
                SQL("COMMENT ON TABLE {} IS {};").format(
                    table_identifier, Literal("created by {} on {}".format(self.storage.user, time.asctime()))
                )
            )
            logger.debug(c.query)
            self.storage.conn.commit()
        ...
        with self.buffered_layer_insert(table_identifier=table_identifier, columns=[c[0] for c in columns]) as insert:
            i = 0
    ...
    def _repr_html_(self):
        if self._is_empty:
            ...
            structure_html = pandas.DataFrame.from_dict(
                self._structure.structure,
                orient="index",
                columns=["layer_type", "attributes", "ambiguous", "parent", "enveloping", "meta"],
            ).to_html()
        ...
        if column_meta is not None:
            ...
            if column_meta:
                meta_html = pandas.DataFrame.from_dict(column_meta, orient="index", columns=["data type"]).to_html()
            else:
