...
class MeasurementTagger(Tagger):
    ...
    def __init__(self, output_layer="measurements", layer_of_tokens="measurement_tokens"):
        ...
        output_attributes = ["name", "OBJECT", "VALUE", "UNIT", "MIN", "MAX", "DATE", "REGEX_TYPE", "SUBJECT"]
    ...
    def measurement_validator(nodes):
        ...
        for node in nodes:
            ...
            if node.name == "MO":
                if node["regex_type"][0] == "PIKKUS":
                    pikkus = True
                elif node["regex_type"][0] == "KAAL":
                    kaal = True

            if node.name == "QNUMBER":
                if pikkus == True and node["unit_type"] == "height":
                    return True
                elif kaal == True and node["unit_type"] == "weight":
                    return True
                else:
                    if pikkus == False and kaal == False:
