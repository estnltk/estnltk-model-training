...
from .taggers.type4_start_end_tagger import Type4StartEndTagger
from .taggers.type5_start_end_tagger import Type5StartEndTagger
...
# prefix="run_202011281548_"
# conf="configurations/egcut_epi_microrun.ini"
# luigi --scheduler-port 8082 --module cda_data_cleaning.data_cleaning.analysis_data_cleaning.step01_analysis_printout_extraction.extract_analysis_text ExtractAnalysisText --config=$conf --role=egcut_epi_work_create --prefix=$prefix --source-schema=original --target-schema=work --target-table=$targettable --workers=1 --log-level=WARNING


class ExtractAnalysisPrintout(CDASubtask):
    """
    ...
    def run(self):
        self.log_current_time("Extract analysis printout ending time")
        self.mark_as_complete()


class ExtractTablesFromTextField(CDASubtask):
    """
    Searches for analysis texts in source table's 'text' field and extract them to target table.
    """
    ...
    target_schema = luigi.Parameter(default="work")
    target_table = luigi.Parameter()
    field = luigi.Parameter()  # e.g. 'original_text'
    luigi_targets_folder = luigi.Parameter(default=".")
    requirement = luigi.TaskParameter()
    ...
    def run(self):
        ...
        block_size = 1000  # 0  # how many rows we fetch from the database
        buffer_size = 100  # 500 #1000 #10000 # how many texts we send to database at once
