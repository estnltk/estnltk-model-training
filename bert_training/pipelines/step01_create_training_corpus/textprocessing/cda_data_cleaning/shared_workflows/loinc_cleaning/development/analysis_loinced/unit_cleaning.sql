...
insert into analysiscleaning.parameter_unit_to_loinc_unit_mapping (parameter_unit)
select distinct clean_unit from analysiscleaning.temp_unit_cleaning as uc,analysiscleaning.temp_possible_units as p
...
update analysiscleaning.parameter_unit_to_loinc_unit_mapping as p
  ...
  where upper(p.parameter_unit) = upper(l.unit);
...
where upper(parameter_unit) = upper(t_yhik);

--null mappingul pole mõtet
delete from analysiscleaning.parameter_unit_to_loinc_unit_mapping
where loinc_unit is null;

select parameter_unit, loinc_unit, evidence from analysiscleaning.parameter_unit_to_loinc_unit_mapping
where    parameter_unit = 'nmol/L';
--group by parameter_unit, loinc_unit;
...
select count(*) from
  ...
  (
      select p.loinc_unit, a.parameter_unit_raw, p.evidence, a.* from analysiscleaning.analysis_html_loinced_runfull201904041255 as a
        left join analysiscleaning.parameter_unit_to_loinc_unit_mapping as p on
        a.parameter_unit_raw = p.parameter_unit
  ) as tbl
where loinc_unit is not null;

-- loinc unit on olemas 2412672 real
...
alter table analysiscleaning.temp_possible_units
...
update analysiscleaning.temp_possible_units as p
    set cleaned_unit = u.clean_unit
    from analysiscleaning.temp_unit_cleaning as u
...
--clean_unitis asendame 24h -> d ja
update analysiscleaning.temp_possible_units
...
select sum(count) from analysiscleaning.temp_possible_units
...
select parameter_unit_raw, count from analysiscleaning.temp_possible_units
where loinc_unit is null
order by count desc;
...
insert into analysiscleaning.parameter_unit_to_loinc_unit_mapping (parameter_unit, loinc_unit, evidence)
    ...
    from  analysiscleaning.temp_possible_units as p
...
insert into analysiscleaning.parameter_unit_to_loinc_unit_mapping(parameter_unit, loinc_unit, evidence)
    ...
    from classifications.elabor_analysis as e, analysiscleaning.temp_possible_units as p
    where upper(parameter_unit_raw) = upper(t_yhik);
...
------------------------------------------------------------------------
-- palju uniteid ta ära mapib hetkel
------------------------------------------------------------------------
